...
class Vehicle_Detector():
    def __init__(self, weights='yolov3/weights/yolov3-spp-ultralytics.pt', cfg='yolov3/cfg/yolov3-spp.cfg', names='yolov3/data/coco.names', iou_thres=0.3, conf_thres=0.15, imgsz=512, half=True, device_id='0'):
    ...
    def detect(self, image=cv2.imread('images/3.png'), view_img=False):
        ...
        if view_img:
            ...
            for key in detections.keys():
                ...
                start = tuple(np.asanyarray(b[:2], dtype=np.int32))
                end = tuple(np.asanyarray(b[2:], dtype=np.int32))
                image = cv2.rectangle(image, start, end, color, thickness=thickness)
