...
from yolov3.detect1 import Vehicle_Detector

image_path = 'Validation_Set/images'
result_path = 'results_cars'
...
total_runs = len(weight_cfg_pairs)*len(iou_thresholds)*len(confidence_thresholds)*len(img_sizes)
    ...
    for iou in iou_thresholds:
        for conf_thresh in confidence_thresholds:
            for imgsz in img_sizes:
                ...
                weight = pair[0]
                cfg = pair[1]
                output_local_dir = weight[15:]+'_iou_'+str(iou)+'_conf_thresh_'+str(conf_thresh)+'_img_size_'+str(imgsz)
                output_path1 = os.path.join(res_subdirectory1, output_local_dir)
                output_path2 = os.path.join(res_subdirectory2, output_local_dir)
                output_path3 = os.path.join(res_subdirectory3, output_local_dir)
                ...
                if not os.path.isdir(output_path3):
                    ...
                    rmtree(output_path3)
                    os.mkdir(output_path3)
                detector = Vehicle_Detector(weights=weight, cfg=cfg, iou_thres=iou, conf_thres=conf_thresh, imgsz=imgsz)
                
                for file_num in range(len(imnames1)):
                    filename = imnames1[file_num]
                    file_save_path = os.path.join(output_path1, filename+'.json')
                ...
                for file_num in range(len(imnames2)):
                    ...
                    file_save_path = os.path.join(output_path2, filename+'.json')
                    f = open(file_save_path, 'w+')
                ...
                for file_num in range(len(imnames3)):
                    ...
                    file_save_path = os.path.join(output_path3, filename+'.json')
                    f = open(file_save_path, 'w+')
