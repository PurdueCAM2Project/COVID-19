...
if __name__ == "__main__":
    ...
    if os.path.exists(filename):
        ...
        with open(filename, 'r') as detections:
            detections = json.load(detections)
        original_numcams = len(list_cams)
        cameras_done = detections.keys()
        for camera in cameras_done:
            list_cams.remove(camera)
        new_numcams = len(list_cams)
        count = original_numcams - new_numcams
    else:
        detections = dict()
        count = 0
    for foldername, image_links, time in i.get_subset_images(cam_list=list_cams):
        ...
        if len(image_links) > 0 and not check:
            ...
            f.close()
            count += 1
            print(f"{count} out of {len(list_cams)} cameras done.")
    print("Done")
